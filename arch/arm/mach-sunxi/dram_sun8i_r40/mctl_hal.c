...
unsigned int dram_vol_set( __dram_para_t *para)
{
	...
	if(ret_val){
		...
		dram_dbg_0("DRAM_VCC set to %d mv\n",vol_val);
		return vol_val;
	}
#else
	return 1;
...
{
	dram_dbg_0("Set master priority\n");
...
void auto_set_timing_para(__dram_para_t *para)
{
	...
	{
		dram_dbg_0("User define timing parameter\n");
		//dram_tpr0
	...
	{
		dram_dbg_0("Auto calculate timing parameter\n");
		if(type==3)
		{
			//dram_tpr0
			tccd=2;
...
unsigned int auto_scan_dram_rank_width(__dram_para_t *para)
{
	 ...
	 dram_dbg_0("Auto scan dram rank width\n");
